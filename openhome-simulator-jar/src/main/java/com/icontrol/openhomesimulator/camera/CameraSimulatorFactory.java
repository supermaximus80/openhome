...
//import com.icontrol.ohsimrest.ResourceIf;
import com.icontrol.ohcm.OpenHomeCameraDriver;
import com.icontrol.openhome.data.bind.LocalDataBinder;
...
public class CameraSimulatorFactory {
    ...
    private LocalRestClient localRestClient = null;
    private Document api;
    private XPath xPath;
    private TreeSet<String> apiMap = new TreeSet<String>();
    ...
    private CameraSimulatorFactory() {
        ...
        try{
            localRestClient = new LocalRestClient("",restServiceInst);
            
            RestClient.Response response=localRestClient.GET("OpenHome/api/doc");
            api= LocalDataBinder.getDocument(new StringReader(Utilities.getCleanXml(response.getContentAsString())));
            xPath = XPathFactory.newInstance().newXPath();
            NodeList nodeList = (NodeList)xPath.evaluate("//@path",api,XPathConstants.NODESET);
            if (nodeList.getLength()!=0){
                for (int i=0; i<nodeList.getLength();i++){
                    if (!nodeList.item(i).getTextContent().contains("example"))
                        apiMap.add(nodeList.item(i).getTextContent());
                }
            }
        }
        catch (Exception e){
            System.out.println("Failed to load api doc.");
            e.printStackTrace();
        }
    ...
    */
    ...
    public TreeSet<String> getApiMap()
    {
        TreeSet<String> set = new TreeSet<String>();
        set.addAll(OpenHomeCameraDriver.defaultCommandMap.keySet());
        return set;
    }

    public Document getCameraApi(){
        return api;
    }
    ...
    public LocalRestClient getLocalRestClient() {
    ...
    public void setContextPath(HttpServletRequest request) throws IOException {
    ...
    public String getCameraVideoStreamingURL(String id) {
        ...
        return "rtsp://" + "relaysirius3.icontrol.com/OpenHome/" + "Streaming/channels/"+id+"/rtsp";
    }
